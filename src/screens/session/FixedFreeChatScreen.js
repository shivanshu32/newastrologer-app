...
/**
 * FixedFreeChatScreen - Production-Ready Free Chat Implementation for Astrologer App
 */
const FixedFreeChatScreen = ({ route, navigation }) => {
  ...
  console.log('🚀 FixedFreeChatScreen (Astrologer): Component mounting with ID:', componentIdRef.current);
  console.log('🚀 FixedFreeChatScreen (Astrologer): Mounting guard status:', mountingGuardRef.current);
  console.log('🚀 FixedFreeChatScreen (Astrologer): Route params:', route.params);
  ...
  // Extract free chat parameters
  const { freeChatId, sessionId, userId, astrologerId = authUser?.id, sessionDuration = 180 } = route.params || {};
  ...
  useEffect(() => {
    ...
    console.log('🚀 [FREE_CHAT_INIT] Starting component initialization for:', componentIdRef.current);
    console.log('🚀 [FREE_CHAT_INIT] FreeChatId:', freeChatId, 'SessionId:', sessionId);
    console.log('🚀 [FREE_CHAT_INIT] UserId:', userId, 'AstrologerId:', astrologerId);
    console.log('🚀 [FREE_CHAT_INIT] AuthUser:', authUser);
  ...
  }, [freeChatId, sessionId, authUser?.id]);
  ...
  }, [freeChatId, sessionId, getCurrentRoomId, messages, safeSetState]);
  ...
  const handleIncomingMessage = useCallback((messageData) => {
    ...
    if (socket?.connected && newMessage.id && newMessage.senderType === 'user') {
      socket.emit('free_chat_message_read', {
  ...
  }, [generateMessageId, freeChatId, sessionId, userId, authUser?.id, getCurrentRoomId, safeSetState]);
  ...
  const handleTypingStarted = useCallback((data) => {
    ...
    // Only handle typing from user in free chat
    if (data.senderType === 'user' && data.freeChatId === freeChatId) {
  ...
  const handleSessionStarted = useCallback((data) => {
    console.log('🚀 [FREE_CHAT_SESSION] Session started event received:', data);
    console.log('🚀 [FREE_CHAT_SESSION] Current freeChatId:', freeChatId);
    console.log('🚀 [FREE_CHAT_SESSION] Event freeChatId:', data.freeChatId);
    console.log('🚀 [FREE_CHAT_SESSION] Current session active state:', sessionActive);
    console.log('🚀 [FREE_CHAT_SESSION] Current timer data:', timerData);
    ...
    console.log('🚀 [FREE_CHAT_SESSION] New free chat session start');
    safeSetState(setSessionActive, true);
    safeSetState(setConnectionStatus, 'connected');
    
    // Initialize 3-minute free chat timer
    const duration = sessionDuration; // 180 seconds for free chat
    console.log('⏱️ [FREE_CHAT_SESSION] Starting 3-minute free chat timer');
    ...
    // Start local timer for free chat
    startLocalTimer(duration);
  }, [safeSetState, startLocalTimer, freeChatId, sessionDuration, saveSessionState, sessionActive, timerData, syncTimerFromSession]);
  
  const handleTimerUpdate = useCallback((data) => {
    console.log('⏱️ [FREE_CHAT_TIMER] Backend timer update received:', data);
    
    if (data.freeChatId !== freeChatId) {
      console.log('⚠️ [FREE_CHAT_TIMER] Ignoring timer for different free chat');
      return;
    }
    ...
    if (data.elapsed !== undefined || data.durationSeconds !== undefined || data.duration !== undefined) {
      ...
      console.log('⏱️ [FREE_CHAT_TIMER] Syncing with backend - elapsed:', backendElapsed, 'duration:', totalDuration);
      ...
      console.log('✅ [FREE_CHAT_TIMER] Successfully synced with backend timer');
    } else {
      console.log('⚠️ [FREE_CHAT_TIMER] No elapsed, durationSeconds, or duration in backend data:', data);
    }
  }, [freeChatId, safeSetState, saveSessionState]);
  ...
  }, [sessionDuration, formatTime, safeSetState, stopLocalTimer, navigation]);
  ...
  const setupSocketListeners = useCallback(() => {
    ...
    socket.on('connect', () => {
      console.log('✅ [FREE_CHAT_SOCKET] Connected successfully');
      console.log('🔥 [FREE_CHAT_SOCKET] Socket ID:', socket.id);
      safeSetState(setConnectionStatus, 'connecting');
      joinFreeChatRoom();
    });
    
    socket.on('disconnect', (reason) => {
      console.log('❌ [FREE_CHAT_SOCKET] Disconnected:', reason);
      safeSetState(setConnectionStatus, 'connecting');
      roomJoinedRef.current = false;
      
      // Trigger auto-reconnection for all disconnect reasons
      if (mountedRef.current) {
        console.log('🔄 [FREE_CHAT_SOCKET] Triggering auto-reconnection due to disconnect');
        setTimeout(() => {
    ...
    socket.on('free_chat_message', (data) => {
      console.log('🔥 [FREE_CHAT_MESSAGE] free_chat_message event fired! Data:', data);
    ...
    console.log('🎯 [FREE_CHAT_SOCKET] Registering free chat session event listeners');
    socket.on('free_chat_session_started', handleSessionStarted);
    
    // Listen to free chat timer updates
    socket.on('free_chat_timer_update', handleTimerUpdate);
    socket.on('free_chat_session_ended', handleSessionEnded);
    socket.on('free_chat_session_resumed', handleSessionStarted);
